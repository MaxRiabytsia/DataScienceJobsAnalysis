...
def insert_into_table(cursor, url, job_info):
    insert_row_command = (
        "INSERT IGNORE INTO raw_jobs (url, job_info) \n"
        "		VALUES (%s,%s)")
    row_to_insert = (url, job_info)
...
def append_from_df_to_db(cursor, df):
    for i, row in df.iterrows():
        insert_into_table(cursor, row['url'], row['job_info'])
